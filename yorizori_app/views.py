...
def addSource(request):
    ...
    if request.method == "GET":
        ...
        new_mylist = mylist.replace('"', '').replace('[', '').replace(']', '').replace(',', '')
        print(new_mylist)
        url = "https://search.naver.com/search.naver?where=view&sm=tab_jum&query=" + new_mylist +"요리"
        print("생성url: ", url)
        # ConnectionError방지
        headers = {
        ...
        c.clear()
        for i in temp:
            if set(map(lambda x: x.strip(), i["source"].split(','))) == set(n_s):
                c.append(i)
